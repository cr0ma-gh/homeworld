...
/* On Android these are real functions provided by gl4es, not loadable
   pointers, so we must not define them as variables here. */
...
#ifdef __ANDROID__
#include <gl4esinit.h>
/* gl4es (NOEGL) asks us for the default framebuffer size; SDL owns the GLES2
   surface, so report the drawable size of our window. */
static void gokGetMainFBSize(int *width, int *height) {
...
#ifdef __ANDROID__
    /* Render at the device's native resolution so the logical UI space matches
       the physical screen (otherwise the engine lays out at 1024x768 and only
       fills a sub-region of the display, which also breaks touch mapping). */
    {
        SDL_DisplayMode dm;
        if (SDL_GetCurrentDisplayMode(0, &dm) == 0 && dm.w > 0 && dm.h > 0)
        {
            MAIN_WindowWidth  = dm.w;
            MAIN_WindowHeight = dm.h;
        }
    }
#endif
...
#ifdef __ANDROID__
    /* gl4es built with NO_INIT_CONSTRUCTOR + NOEGL: wire the proc loader and
       framebuffer-size callback, then initialise it now that SDL has a current
       GLES2 context. After this, the engine's legacy gl* calls go through gl4es. */
    set_getprocaddress((void *(*)(const char *))SDL_GL_GetProcAddress);
    set_getmainfbsize(gokGetMainFBSize);
    initialize_gl4es();
#endif
...
#if defined(HW_ENABLE_GLES) || defined(__ANDROID__)
        /* VBOs are core in GLES / provided by gl4es */
        return 1;
#else
...
{
#ifdef __ANDROID__
    /* Universal cursor draw: rndMainViewRenderFunction does call mouseDraw, but
       some render paths (post-NIS dialogue, certain front-end screens) skip it
       and the cursor stays hidden until the next "normal" frame. Drawing here,
       just before the swap, guarantees the user always sees a cursor on a touch
       build regardless of which path produced the frame. */
    mouseDraw();
    {
        extern void gokAtkPerFrame(void);
        gokAtkPerFrame();   /* defers CTRL+SHIFT release after a band-attack drag */
    }
#endif
    glFlush();
    primErrorMessagePrint();
