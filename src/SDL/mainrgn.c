...
/* Touch builds render at native (high) resolution, so a finger drag maps to a
   much larger mouse delta than the desktop tuning assumed -> scale the camera
   motion delta down so two-finger orbit isn't hyper-sensitive. */
#ifdef __ANDROID__
#define MR_CAMERA_MOTION_NUM 3
#define MR_CAMERA_MOTION_DEN 10
#else
#define MR_CAMERA_MOTION_NUM 1
#define MR_CAMERA_MOTION_DEN 1
#endif
...
void mrCameraMotion(void)
{
    ...
    sdword cx, cy;
#ifdef __ANDROID__
    /* On Android the camera is driven directly from touch (camMouseX/Y set in
       HandleEvent, main.c), bypassing this recentred mouse-look which fights
       with touch input and feels jerky. */
    return;
#endif
    if (helpinfoactive)
    {
        if (!mouseInRect(&helpinforegion->rect))
        {
            ...
            mouseCursorY() != cy)
            {
                ...
                camMouseX = (cx - mouseCursorX()) * MR_CAMERA_MOTION_NUM / MR_CAMERA_MOTION_DEN;
                camMouseY = (cy - mouseCursorY()) * MR_CAMERA_MOTION_NUM / MR_CAMERA_MOTION_DEN;
    ...
    else
    {
        ...
        mouseCursorY() != MAIN_WindowHeight / 2)
        {
            ...
            camMouseX = (MAIN_WindowWidth / 2 - mouseCursorX()) * MR_CAMERA_MOTION_NUM / MR_CAMERA_MOTION_DEN;
            camMouseY = (MAIN_WindowHeight / 2 - mouseCursorY()) * MR_CAMERA_MOTION_NUM / MR_CAMERA_MOTION_DEN;
    ...
    switch (event)
    {
       ...
       case RPE_PressLeft:
        ...
        case RPE_PressRight:
            ...
            mrOldMouseY = mouseCursorY();
#ifndef __ANDROID__
            /* Desktop RMB-camera hides the cursor and warps it to the screen
               centre for relative mouse-look. On Android we drive the camera
               from touch deltas (camMouseX/Y) and have nothing equivalent to
               recenter, so keep the cursor visible and put-where-the-user-had-it
               throughout the camera gesture. */
