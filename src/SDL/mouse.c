...
#ifdef __ANDROID__
    /* On touch the engine's many cursor-hide paths (camera mouse-look, sensor
       manager, NIS/dialogue lockout, animatic, plug screens) are for desktop
       habits that just make the cursor disappear here -- force it visible
       every frame and skip the mouseDisabled gate so a single-player dialog
       can't strand the user without a cursor when it ends. */
    mouseIsVisible = TRUE;
#else
...
#ifdef __ANDROID__
    /* On touch the cursor must always be a real, drawable one. Camera mouse-look,
       NIS lockout, ESC-skipping a dialogue and unknown/uninitialised states all
       leave mouseCursorType stuck somewhere that draws nothing or hits the
       switch's default branch (NULL texture). Force a normal cursor every frame
       so the user is never stranded without one. */
    mouseCursorType = normal_mouse;
#endif
