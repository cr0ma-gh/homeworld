      ...
      for(j=0; j<oldfescreen->nAtoms;j++,newfeatom++,oldfeatom++){
        ...
        if (oldfeatom->pData != 0){
          if (oldfeatom->type == FA_RadioButton){
            ...
            newfeatom->pData = (ubyte *)((Uint64)newptr - (Uint64)newrootptr );
            newptr = stpcpy_reimplementation(newptr,  (char *)((Uint64)oldfeatom->pData + oldrootptr) );
            newptr = (void*)(((Uint64)newptr + 8) & ((Uint64) ~7));
          }
        }
        if (oldfeatom->attribs != 0){
          newfeatom->attribs = (ubyte *)((Uint64)newptr - (Uint64)newrootptr );
...
fibfileheader *feScreensLoad(char *fileName)
{
    ...
    for (screenIndex = 0; screenIndex < header->nScreens; screenIndex++, screen++)
    {
        ...
        screen->links = (felink *)((memsize)loadAddress + (memsize)screen->links);      //update list pointers
        screen->atoms = (featom *)((memsize)loadAddress + (memsize)screen->atoms);
        for (index = 0; index < screen->nLinks; index++)
